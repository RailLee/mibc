...
> ctDNA 应作为二值、连续值，还是多维特征？阳性/阴性用于临床解释，VAF/甲基化 score 用于连续建模，突变/甲基化位点向量用于探索性特征学习。（不知道对不对）
...
| 编号                     | 精炼后的研究内容                                                                                                         |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------- |
| **RQ1：纵向 ctDNA 动态建模**  | 如何将 T0–T4 多时间点 ctDNA 变异、VAF、甲基化 score、清除、持续阳性和反弹信号，转化为可解释的疗效与复发风险轨迹？                                         |
| **RQ2：WSI-MIL 病理表征学习** | 能否从治疗前 TURBT 活检和/或术后切除标本的 WSI patch-level features 中，学习与 pCR、MPR、MRD 和 RFS 相关的 patient-level representation？ |
| **RQ3：节点自适应多模态融合**     | 在 T0、T1、T2、T3、T4 各节点可用模态不同的情况下，模型能否自动利用当前可获得的 ctDNA、WSI、影像、膀胱镜和临床信息，而不要求所有模态齐全？                              |
| **RQ4：模态互补性？**    | 每种模态分别提供什么独立信息，在哪个治疗阶段最有预测价值，加入融合模型后是否真正提高 pCR/MPR 和 RFS 预测性能？                                               |
| **RQ5：多任务建模**        | 疗效响应、MRD 状态和复发风险是否应被建模为连续临床链条，并通过多个输出头分别预测 pCR/MPR、MRD 高风险和 RFS/OS？                                          |
| **RQ6：低成本检测策略优化**      | 在检测成本受限的情况下，能否筛选最有信息量的 ctDNA 采样时间点、样本来源和甲基化位点组合，在尽量少检测的前提下保留主要预测性能？                                          |

关于第六个问题，是不是可以被写成成本约束下的检测策略优化。（不知道可不可这么写）设 $S_T$ 表示选择的采样时间点，例如 T0、T1、T2、T3、T4；$S_M$ 表示选择的甲基化位点或 DMR 区域；$S_B$ 表示选择的样本来源，例如 plasma、urine sediment、urine supernatant。优化问题

$$
\max_{S_T,S_M,S_B} \; \mathrm{Performance}(S_T,S_M,S_B) - \lambda \mathrm{Cost}(S_T,S_M,S_B)
$$

其中，$\mathrm{Performance}$ 可以选择 AUC、C-index、NRI 等预测性能指标，$\mathrm{Cost}$ 表示检测成本，$\lambda$ 是成本惩罚系数。更临床化的表述是，在预测性能接近全量检测模型的前提下，寻找成本最低的检测组合：

$$
\min_{S_T,S_M,S_B} \; \mathrm{Cost}(S_T,S_M,S_B)
$$

$$
\text{s.t.} \quad \mathrm{AUC}(S_T,S_M,S_B) \geq \mathrm{AUC}_{\mathrm{full}} - \epsilon
$$

其中，$\mathrm{AUC}_{\mathrm{full}}$ 表示全量检测模型的性能，$\epsilon$ 表示可接受的性能下降幅度。该设计的短期目标是减少不必要检测，长期目标是提高单位检测成本下的临床获益。
